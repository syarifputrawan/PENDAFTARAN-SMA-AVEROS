...
$id = $_GET['id'];
$selectID = "SELECT id, gambar_rektor, sambutan_rektor FROM about WHERE id=$id";
...
<main>
    <div class="container-fluid px-4">
        <h1 class="mt-4">ubah data sambutan dan gambar kepsek</h1>
        <ol class="breadcrumb mb-4">
            <li class="breadcrumb-item">tentang</li>
            <li class="breadcrumb-item active">ubah data sambutan dan gambar kepsek</li>
        </ol>
        <form action="<?= BASE_URL . 'aksi/tentang/ubah_info_sambutan_rektor.php' ?>" method="post" enctype="multipart/form-data">
            <input type="hidden" name="id" value="<?= $data['id'] ?>">
            <div class="form-group mb-3">
                <label for="">gambar rektor</label> <br>
                <img id="gambarSejarah" width="100" height="100" class="rounded"> <br>
                <input type="file" name="gambar_rektor" class="form-control" onchange="gambarSejarahDipilih(event)">
            </div>
            <div class="form-group mb-3">
                <label for="">sambutan kepsek</label> <br>
                <textarea name="sambutan_rektor" class="buatText form-control" cols="30" rows="5"><?= $data['sambutan_rektor'] ?></textarea>
            </div>
...
<script>
    ...
    gambarSejarah.src = "<?= BASE_URL . 'images/tentang/' . $data['gambar_rektor'] ?>";
    // #####---------
